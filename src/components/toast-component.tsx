...
const ToastContainer = styled('div')((props: ToastContainerProps) => {
    const { isClosing, position, direction, style, type } = props;
    return `
      will-change: transform;
      animation: ${isClosing ? fadeOut(position) : fadeIn(position)} 0.35s ease-in-out;
      direction: ${direction};
...
const ExpandedContent = styled('div')(
    ...
    max-height: ${props.isExpanded ? props.expandedHeight : '0px'};
  `
);
...
const Toast: React.FC<ToastProps & { onClose: () => void }> = React.memo(
    ({
        ...
        const toastFunctions = {
            update: (updates: Partial<Toast>) => updateToast(id, updates),
